...
class CogCmd::Ecs::Task::Register < Cog::SubCommand
  ...
  Options:
    ...
    --replace-volumes                     When true existing volumes are replaced.
                                          Note: Only valid with --use-previous-revision
                                          By default if a volume is declared with the same name it is replaced
                                          otherwise the volume is appended to the list.
    --use-previous-revision, -p           Use the previous revision as a base.
    --replace-definitions                 When true existing container definitions are replaced.
                                          Note: Only valid with --use-previous-revision
                                          By default definitions are only replaced if one with the same
                                          name is specified. Otherwise definitions are appended to the list.
  END
